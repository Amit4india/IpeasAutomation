...
public class HttpSnmpTrapTests {

	@Test
	public void checkHttpTrap() throws InterruptedException {
		System.out.println("Staring checkHttpTrap for East");
		Util.simulateStopHttp(eastAutomationAgent);
		...
		Assert.assertEquals(true, (WebDriverUtil.getSingleRow(tr_collection.get(0)).get(4).getText().contains(searchHttpdStopSnmp )));
		Util.simulateStartHttp(eastAutomationAgent);
		WebDriverUtil.findElementByXpath(searchFieldBoxInSplunk).clear();
		System.out.println("checkHttpTrap done for East successfully");
