...
public class NTPSnmpTrapTestsForWest {
	
	@Test
	public void checkNTPTrap() throws InterruptedException {
		System.out.println("Starting checkNTPTrap for West");
		Util.simulateStopNtp(westAutomationAgent);
		WebDriverUtil.findElementByXpath(searchFieldBoxInSplunk).clear();
		WebDriverUtil.insertInTextBox(searchFieldBoxInSplunk, searchNtpdStopSnmp);
		...
		Assert.assertEquals(true,  (WebDriverUtil.getSingleRow(tr_collection.get(0)).get(4).getText().contains(searchNtpdStopSnmp)));
		Util.simulateStartNtp(westAutomationAgent);
		WebDriverUtil.findElementByXpath(searchFieldBoxInSplunk).clear();
		System.out.println("checkNTPTrap done for West Glue Server successfully");
